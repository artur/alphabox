...
| `nt_snap.sh` | `make`: boot the install once and snapshot the logged-in desktop (`SIGUSR1` saves with all threads stopped; `ALPHABOX_SNAPSHOT`, `ALPHABOX_SNAPSHOT_EXIT`). `run`: resume it (`ALPHABOX_RESTORE`) and run a benchmark section, seconds instead of a two-minute boot. `perf_ab.py --snapshot` drives it. |
