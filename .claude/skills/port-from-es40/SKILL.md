---
name: port-from-es40
description: Evaluate upstream ES40-Emu changes and selectively adopt the good ones into alphabox. Use whenever the user asks to compare with, bring in, or port changes from the es40 repo (github ES40-Emu/es40). Covers triage and critical review, delta computation, manual adoption vs reviewed whole-file takes, the alphabox invariants (std:: threading, .hpp, clang-format, rebrand), and the verification gates.
---


# Evaluating and adopting ES40-Emu changes in alphabox

alphabox (this repo) is its own project; the ES40-Emu emulator
(https://github.com/ES40-Emu/es40, clone it locally, e.g. to /tmp/es40/)
is a source of candidate fixes. **Neither upstream nor alphabox is the source
of truth -- the goal is the best code, not parity.** For every upstream
change: review it critically against alphabox's code (and the hardware
...
apply: alphabox is clang-formatted, renames headers `.h` → `.hpp`,
...
build, docs) and triage each: what it fixes, whether alphabox already has it
or an equivalent, value, conflict risk with alphabox-only work, effort, and a
verdict (adopt / adapt / improve / reject / already-have). Verify the
claimed bugs in alphabox's own source before acting. For large rewrites, do a
critical code review (correctness vs hardware, threading, savestates, debug
noise, dead code) before deciding what to take. Plan roughly one alphabox
commit per theme. Every commit must build in all lanes and pass the SRM
test.

Files to ALWAYS skip (alphabox does not have them):
`src/Visual Studio/*`, `configure.ac`, `config_vms.h`,
`config_win32.h`, `.github/ISSUE_TEMPLATE/*`. Upstream `CMakeLists.txt`
changes almost never apply — alphabox has its own; read the upstream
hunk and decide if the *idea* (a new source file, a new dependency) is
needed. alphabox versioning is its own (`project(Alphabox VERSION x.y.z)`
in CMakeLists.txt) — NEVER adopt upstream's version number.

## Step 1 — file mapping (upstream path → alphabox path)

Upstream keeps every source in a flat `src/`; alphabox does not (reorganized
2026-09-16). Never assume a path — find the alphabox home of a file with
...
- `src/es40-cfg.cpp` → `src/es40-cfg.cpp` — entry points stay at `src/`
  root — but its `main()` is named `main_cfg()` (single `alphabox` binary;
  `src/Main.cpp` dispatches `alphabox run` → `main_sim`,
  `alphabox configure` → `main_cfg`).
...
- New upstream headers: create as `.hpp` in the matching directory,
  ...
  alphabox license header (see Step 4). A new `.cpp` in an EXISTING
...
# (b) what alphabox changed relative to the last ported upstream state:
git -C $E show $BASE:src/F > $S/F.base
diff -u $S/F.base <(git show <preformat>:src/F_alphabox_path)
```

For (b) you need the alphabox snapshot from BEFORE the repo-wide
...
- **Reviewed whole-file take (W)** — only when a critical review found
  ...
  1. `git -C $E show HEAD:src/F > <alphabox path of F>` (see Step 1 — it
     is not `src/F` any more).
  2. `sed`-rename every `#include "X.h"` to `"X.hpp"`. Verify none
     remain: `grep -n '\.h"' src/F | grep -v '\.hpp"'`.
  3. Re-apply the alphabox delta. Fastest reliable way: save diff (b) to
...
2. **Preserve the alphabox-only bug fixes** when replacing CPU/JIT code:
   ...
   - Debug env hooks (documented in docs/headless.md):
     `ALPHABOX_PC_SAMPLE` (AlphaCPU::check_state), `ALPHABOX_DUMP_FB`,
     `ALPHABOX_AUTOKEY_ENTER`, `ALPHABOX_KEYSCRIPT`, `ALPHABOX_KEYPIPE`,
     `ALPHABOX_AUTOMOUSE`, `ALPHABOX_MOUSE_DEBUG` (gui/sdl.cpp; the aux-cmd
...
4. **Configurator allow-lists** (`kv_*[]` arrays in
   ...
   must be added to that device's list, and alphabox-only keys must
...
All HOST-side user-visible text says **Alphabox**, never ES40:
window titles (sdl/x11/win32), the startup banner
(`src/common/banner.hpp` — `print_alphabox_banner`, shows alphabox's own
...
alphabox, keep the lineage header its alphabox copy already carries:
...
header — match whatever the file's current alphabox copy has. Strip
...
5. **Probes and guests for the area you changed**: SRM probes from the
   ...
   CPUs); `ALPHABOX_IRQSTATS=1` for interrupt changes.
6. **Benchmark** for JIT/dispatch changes: Windows 2000 RC2, 2 CPUs,
   `build-jit-stats-sdl`, `ALPHABOX_NO_IDLE=1`, 300 s, per-CPU MIPS p50
...
9. **Configurator smoke**: `printf 'no\n' | build/alphabox configure`
   shows the Alphabox banner and exits cleanly.

Never `pkill`/`killall alphabox` — other sessions on the host may be running
...
1. **Triage first, read-only.** For each phase, have an agent compare the
   upstream commits with alphabox's code and report per commit: what it
   does, alphabox status with file:line, merit (including bugs in the
...
which upstream commits + which alphabox deltas were preserved/dropped.
...
- `7140555..9f7554d` (v0.75.1→0.75.4) — 4 commits (2026-07-11):
  ...
  live only in this fork and lenticularis's AXPbox) — port upstream DEC21143 pcap
...
- Post-port alphabox-only additions (2026-07-10, keep across future
  ports): input-injection/diagnostic hooks (ALPHABOX_KEYPIPE,
  ALPHABOX_AUTOMOUSE, ALPHABOX_MOUSE_DEBUG + aux-cmd trace in
...
- `9f7554d..2aa5e11` (v0.75.4→v0.85, 254 commits) -- reviewed on merit
  (2026-09-15), on branch arm64-jit (fork artur/axpbox, since renamed alphabox), in phases:
...
- Post-review fixes (2026-09-16), from the open-issue list rather than a
  ...
  underflow traps (ALPHABOX_JIT_FPTEST 42104 failures -> 0), and expected
