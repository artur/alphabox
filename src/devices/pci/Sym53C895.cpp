 ...
 *
 * Called for guest-initiated starts (DSP write, DCNTL.STD, SIGP resume of
 * WAIT RESELECT) and for SCRIPTS writing DSP/DCNTL themselves, in which case
 * the running program simply continues from the new DSP.
 **/
void CSym53C895::start_scripts() { state.executing = true; }
 ...
 **/
void CSym53C895::run_scripts_inline() {
 ...
 * disk I/O) and Memory Moves copy arbitrary amounts of guest memory; neither
 ...
 **/
bool CSym53C895::inline_can_execute_next() {
  u32 insn;
  try {
 ...
 * Caller holds myRegLock.
 **/
void CSym53C895::halt_scripts_on_failure(const std::string &msg) {
...
void CSym53C895::check_state() {
  ...
  // Runs on the main thread: take myRegLock so the GP-timer RAISE() and the
  // disconnect countdown below don't race SCRIPTS. RAII unlock covers the
  // early returns (and the throw) below.
