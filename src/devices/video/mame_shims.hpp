...
#ifdef _WIN32 // this was a workaround for .... I think it was ... uh, windows.h
              // being annoying.
#ifndef NOMINMAX
#define NOMINMAX
#endif
...
public:
  ...
  uint32_t &pix(int y, int x = 0) { return m_pixels[(size_t)y * m_width + x]; }

  uint32_t *rowptr(int y) { return m_pixels.data() + (size_t)y * m_width; }
  ...
  rectangle cliprect() const {
    return rectangle(0, m_width - 1, 0, m_height - 1);
  }
...
public:
  ...
  void configure(int htotal, int vtotal, const rectangle &visarea,
                 int64_t refresh) {
    m_visarea = visarea;
...
public:
  ...
  template <typename T> T *subdevice(const char * /*tag*/) { return nullptr; }
...
public:
  ...
  // Set the target (called during ES40 init instead of MAME's device
  // resolution)
  ...
  template <typename U> void set_tag(U && /*tag*/) {} // template overload
...
#define DEFINE_DEVICE_TYPE(Type, Class, ShortName, FullName) const int Type = 0;

#define DECLARE_DEVICE_TYPE(Type, Class) extern const int Type;
...
inline uint8_t pal6bit(uint8_t v) { return (v << 2) | (v >> 4); }
