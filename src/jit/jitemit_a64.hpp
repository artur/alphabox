...
// The data page cache's second level (CAlphaCPU::data_page_cache2), shared
// by every memory op's cold stub, so the stubs stay a few instructions long
// -- carrying the probe in each one made the compiled code half as big again
// and cost the branch-heavy code more than the memory ops gained
// (docs/performance.md). One per row, the row's offsets built in. Entered
// with blr, x2 = va. On a hit -- the tag matches the live key and the slot
// belongs to the current generation -- the slot is copied into level 1, so
// the next access to the page hits inline, and it returns Z set with x10 =
// the page's bias; Z clear on a miss. Uses x0, x1, x3, x9-x12 and x17, which
// the helper call the stub otherwise makes clobbers too.
void *CJitEngine::a64_dpc2_thunk(bool write_row) {
  void *&thunk = m_dpc2_thunk[write_row ? 1 : 0];
  if (thunk)
    return thunk;
  if (!m_off.dpc2_base || m_off.dpc_stride != 64 ||
  ...
  a.ubfx(a64::x10, a64::x2, imm(13), imm(m_off.dpc2_bits));
  a.mov(a64::x9, imm(l2_row));
  a.add(a64::x9, a64::x9, a64::x10, a64::lsl(6));
  a.add(a64::x9, a64::x19, a64::x9);     // level 2's slot
  a.ldr(a64::x12, a64::ptr(a64::x9, 0)); // its tag
  ...
  a.b_ne(out);
  a.ldr(a64::w0, a64::ptr(a64::x9, (int32_t)m_off.dpc_slot_gen));
  a.ldr(a64::w1, a64_cpu_field(a, m_off.dpc2_gen, 2));
  a.cmp(a64::w0, a64::w1);
  a.b_ne(out);
  ...
  for (int32_t k = 0; k < 64; k += 16) { // loads and stores leave Z alone
    a.ldp(a64::x0, a64::x1, a64::ptr(a64::x9, k));
    a.stp(a64::x0, a64::x1, a64::ptr(a64::x3, k));
  }
  a.ldr(a64::x10, a64::ptr(a64::x9, 8)); // the page's bias
  a.bind(out);
  a.ret(a64::x30);
  if (eh.failed)
    return nullptr;
...
void CJitEngine::emit_op(void *a_ptr, const uint8_t *gpa, void *done_ptr,
  ...
  do {
    ...
    // The second level (CAlphaCPU::data_page_cache2), probed from a cold
    // stub, so only after level 1 has missed: a level-1 hit pays nothing.
