...
#   CTRL_BUS  which of the controller's disk busses the test disk hangs on
#             (default 0); a two-channel part such as the 53C896 has a
#             second one
...
python3 - "$CFG" "$CTRL" "${CTRL_OPTS:-}" "${BRIDGE:-}" "${BRIDGE_DEV:-2}" \
  "${IDE_DEV:-disk0.1}" "${CTRL_BUS:-0}" > storage.cfg <<'PY' || exit 2
import sys
cfg, ctrl, opts, bridge, bridge_dev, ide_dev, bus = sys.argv[1:8]
t = open(cfg).read()
...
if ctrl == "ali_ide":
    ...
    sys.exit()
sec = "  pci0.3 = %s\n  {\n%s%s  }\n\n" % (
    ctrl, ("    " + opts + "\n") if opts else "", drive("disk%s.0" % bus))
if bridge:
