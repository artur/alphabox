...
#
# usage: win_bench.sh <label> <axpbox-binary> <install-dir> <cfg> <seconds> [VAR=value ...]
#   install-dir  guest install directory; relative paths are under
#                $AXPBOX_WORK (default <repo>/lab). It is never modified:
#                the run uses a clone in $AXPBOX_WORK/bench-<label>.
#   cfg          configuration file name inside the install directory
#   VAR=value    extra environment, e.g. AXPBOX_NO_IDLE=1, AXPBOX_IRQSTATS=1
#
...
#
# Benchmark caveat: with the guest idle at its desktop and AXPBOX_NO_IDLE=1,
...
cd "$D" || exit 2
env "$@" SDL_VIDEO_DRIVER=dummy AXPBOX_DUMP_FB=fb/fb "$BIN" run "$CFG" > bench.log 2>&1 &
P=$!
