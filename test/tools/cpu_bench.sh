...
small=$D/small.img
large=$D/large.img
python3 "$T/bench_image.py" "$small" --iterations "$ITER" --body "$BODY" > /dev/null
python3 "$T/bench_image.py" "$large" --iterations $((ITER * 2)) --body "$BODY" > /dev/null
...
echo "== $LABEL: $BODY instructions per iteration, $ITER and $((ITER * 2)) iterations"
